...
log_DIR = r'/usr/local/home/ssbw5/classification/crnn/logs/new_im/OU13_tr_val_K5_models.crnn_pretrained_densenet121/logit_vec'
Kfolds = 1
...
for model_num in range(Kfolds):
    ...
    if perform_train:
        ...
        data_val = torch.load(os.path.join(log_DIR,'val_feats_m'+str(model_num)+'.pt'))
        data_te = torch.load(os.path.join(log_DIR,'test_feats_m'+str(model_num)+'.pt'))
        ...
        data = {'train':(data_tr['x'],data_tr['y'],data_tr['patch_count'],data_tr['im_names']),
                ...
                'test':(data_te['x'],data_te['y'], data_te['patch_count'],data_te['im_names'])}
    ...
    if perform_val:
        ...
        data = {'test':(data_val['x'],data_val['y'], data_val['patch_count'], data_val['im_names'])}
        ...
        model_wt_dir = glob.glob(os.path.join(log_DIR,'model_{}'.format(model_num),'*.pth'))
        print(model_wt_dir)
        if torch.cuda.is_available() and with_cuda:
    ...
    overall_im_labels, overall_im_probs, overall_gt,all_f,all_l = ops.infer(model, data['test'], with_cuda=with_cuda,verbose=False)
